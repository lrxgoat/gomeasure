...
type TLSResult struct {
	...
	SCSV        bool   `json:"scsv"`
	Tls13Down12 string `json:"tls_13_down_12"`
	Tls13Down11 string `json:"tls_13_down_11"`
	Tls13Down10 string `json:"tls_13_down_10"`
...
// 进行TLS连接
func scanTLS(conn *net.Conn, serverName string, timeout time.Duration, maxVersion uint16, ciphers []uint16, sni bool) (*tls.Conn, error) {
...
func Scan(tlstarget *TLSResult, certtarget *CertResult, cttarget *CTResult, revoketarget *RevokeResult, tlsf *os.File, certf *os.File, ctf *os.File, revokef *os.File, port string, sni bool) (bool, string) {
	...
	// 建立TLS连接
	tlsConn, err := scanTLS(&conn, domain, timeout, 0,nil, sni)
	if err != nil {
		return false, err.Error()
	}
	...
	if err != nil {
		...
		_, err := scanTLS(newConn, domain, timeout, state.Version-1, scsvCiphers,sni)
		if err != nil {
	...
	// TLS1.3 降级攻击保护
	//if tlstarget.TlsVersion == "TLSv1.3"{
...
func Start(jobs <-chan string, TLSFile string,CertFile string, CTFile string, RevokeFile string, port int, sni bool, wg *sync.WaitGroup, limiter *rate.Limiter, ctx context.Context) {
	...
	for line := range jobs {
		...
		// 开始扫描
		success, err := Scan(tlstarget, certtarget, cttarget, revoketarget, tlsf, certf, ctf, revokef, strconv.Itoa(port), sni)
		// 扫描失败
		if !success {
